...
class UnauthorizedWorkerError extends Error {}
...
function assertWorkerAccess(request: Request) {
  ...
  if (!expectedToken) {
    throw new WorkerConfigurationError("QUANT_WORKER_API_TOKEN is required for worker imports.");
  }

  const actualToken = request.headers.get("x-worker-token");
  if (!actualToken) {
    throw new UnauthorizedWorkerError("Unauthorized worker request.");
  }
...
export async function POST(request: Request) {
  try {
    ...
    const context = await getWorkerImportContext();
    return NextResponse.json(await importResearchRun(context, payload), {
      status: 201,
    });
  } catch (error) {
    const status =
      error instanceof z.ZodError
        ? 400
        : error instanceof UnauthorizedWorkerError
          ? 401
