...
export async function GET() {
  try {
    ...
    return NextResponse.json(
      await loadPreferences(context, hasTenantCapability(context.role, "research", "write")),
    );
  } catch (error) {
...
export async function PUT(request: Request) {
  try {
    ...
    return apiError(
      error,
      error instanceof SmartInsightsInputError || error instanceof SyntaxError ? 400 : 503,
    );
  }
}
