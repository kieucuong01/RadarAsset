...
describe("market data freshness", () => {
  it("labels fixture data explicitly even when recently published", () => {
    expect(
      calculateFreshness({
        market: "crypto_spot",
        timeframe: "1d",
        coverageEnd: new Date("2026-08-09T00:00:00Z"),
  ...
  it("allows one missing daily bar inside the daily tolerance", () => {
    expect(
      calculateFreshness({
        market: "crypto_spot",
        timeframe: "1d",
  ...
  it("rolls the expected FPT bar backward over the weekend", () => {
    ...
    expect(expectedClosedBarOpen("vn_equity", "1d", now)).toEqual(new Date("2026-08-06T17:00:00Z"));
    expect(
      calculateFreshness({
        market: "vn_equity",
        timeframe: "1d",
        coverageEnd: new Date("2026-08-06T17:00:00Z"),
  ...
  it("keeps daily crypto data fresh when the latest closed session is covered", () => {
    expect(
      calculateFreshness({
        market: "crypto_spot",
        timeframe: "1d",
